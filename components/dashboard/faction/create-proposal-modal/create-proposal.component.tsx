										...
										>
											{BLUEPRINTS.map((blueprint) => (
												<option key={blueprint.name} value={blueprint.name}>
													{blueprint.name} / Cost:{" "}
													{blueprint.upgradeResources[0].map((resource, resourceIndex) => (
														<Text key={resourceIndex}>
															{resource.resource} x{resource.amount}
															{resourceIndex !== blueprint.upgradeResources[0].length - 1 &&
																", "}
														</Text>
													))}
												</option>
									...
									<Box mb="2rem" w="100%">
										...
										>
											<NumberInputField
												...
												max={100}
											/>
											<NumberInputStepper pr="3rem" borderColor={colors.blacks[600]}>
												<NumberIncrementStepper
													fontSize="12px"
													borderColor={colors.blacks[600]}
